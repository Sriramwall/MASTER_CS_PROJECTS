...
d3.json("../jsonFiles/cc_ncaa_sport.json", function(data) {
  drawBubbles(data);
});
...
d3.select("#dropdown").on("change", function change() {
  ...
  console.log(display);
  if (display) {
    d3.json("../jsonFiles/cc_ncaa_".concat(display).concat('.json'), function(data) {
      drawBubbles(data);
    });
...
d3.csv('../wordcloudcsv/cc_ncaa.csv', function (data) {
    var leaders = [];
