...
d3.json("../jsonFiles/cc_nba_nba.json", function(data) {
  drawBubbles(data);
});
...
d3.select("#dropdown").on("change", function change() {
  ...
  console.log(display);
  if (display) {
    d3.json("../jsonFiles/cc_nba_".concat(display).concat('.json'), function(data) {
      drawBubbles(data);
    });
...
d3.csv('../wordcloudcsv/cc_nba.csv', function (data) {
    var leaders = [];
