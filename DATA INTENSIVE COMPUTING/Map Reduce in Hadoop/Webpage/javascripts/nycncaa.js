...
d3.json("../jsonFiles/nyc_ncaa_point.json", function(data) {
  drawBubbles(data);
});
...
d3.select("#dropdown").on("change", function change() {
  ...
  console.log(display);
  if (display) {
    d3.json("../jsonFiles/nyc_ncaa_".concat(display).concat('.json'), function(data) {
      drawBubbles(data);
    });
...
d3.csv('../wordcloudcsv/nyc_ncaa.csv', function (data) {
    var leaders = [];
